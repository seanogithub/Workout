...
public class UI_DayPicker : MonoBehaviour
{
    ...
    public void OnOkPressed()
    {
        if (PreviousSelectedDayIndex > 0)
        {
            ...
            if (Valid == true)
            {
                ...
                {
                    PopUpOkDialog("Training sucessfully added to calendar", this.gameObject.GetComponent<UI_DayPicker>(), "GoodDay");
                }
            }
            else
            {
                if (PopUpActive == false)
                {
                    PopUpOkDialog("Please pick a day", this.gameObject.GetComponent<UI_DayPicker>(), "BadDay");
                }
            }
        }
        else
    ...
    public void AddTrainingToCalendar()
    {
        int SelectedTraining = UICreateTraining.GetComponent<UI_CreateTraining>().SelectedTraining;
        ...
        {
            UserBlobManager.GetComponent<UserBlobManager>().UserTrainingData.Training[SelectedTraining].DayArray.Day[DayIndex].Day = new System.DateTime(CurrentYear, CurrentMonthIndex, CurrentDay).AddDays(DayIndex);
            if (UserBlobManager.GetComponent<UserBlobManager>().UserTrainingData.Training[SelectedTraining].DayArray.Day[DayIndex].DayWorkoutArray.Length > 0)
            {
                ...
                for (int addIndex = 0; addIndex < NumWorkoutsToAdd; addIndex++)
                {
                    ...
                    System.DateTime addWorkoutDay = new System.DateTime(CurrentYear, CurrentMonthIndex, CurrentDay).AddDays(DayIndex);
